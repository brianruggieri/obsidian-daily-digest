...
describe("compressActivity", () => {
	...
	it("compresses a small day within budget", () => {
		...
		const result = compressActivity(categorized, searches, claude, [], 3000);

		expect(result.totalEvents).toBe(30 + searches.length + 5);
	...
	it("compresses a heavy day (400+ events) within budget", () => {
		...
		const expectedTotal = 400 + searches.length + 30;
		expect(result.totalEvents).toBe(expectedTotal);
		expect(result.totalEvents).toBeGreaterThan(400);
	...
	it("allocates budget proportionally to source sizes", () => {
		// Browser-heavy day: 200 visits, 5 searches, 0 claude
		const visits = generateBrowserVisits({
			count: 200,
			domains: DOMAIN_SETS.webdev,
		...
		const categorized = categorizeVisits(visits);
		const result = compressActivity(categorized, searches, [], [], 3000);
	...
	it("respects a tiny budget by using stats-only mode", () => {
		...
		const result = compressActivity(categorized, searches, [], [], 500);

		const expectedTotal = 300 + searches.length;
		expect(result.totalEvents).toBe(expectedTotal);
