...
How `summarizeDay()` in `summarize.ts` selects which prompt to build.
...
What happens when `main.ts` writes a note to the vault.
...
= false                      → PatternAnalysis is undefined
                              → Tier 5 (deidentified prompt) is unavailable
